...
"""
# Method 2:
model = keras.models.Sequential([                                            # Define Sequential model with 3 layers with adding initializer & LeakyReLU
    keras.layers.Flatten(input_shape=[28, 28]),                              # Layer input : Input must be vectored
    keras.layers.Dense(100, activation="elu", kernel_initializer=init_1),    # Layer 1: The first hidden layer
    keras.layers.Dense(75),                                                  # Layer 2: The second hidden layer
